...
function CardBg({ variant }) {
  const grain = {
    position:'absolute', inset:0, opacity:0.5, mixBlendMode:'soft-light', pointerEvents:'none',
...
const FONT_MAP = { Display: "var(--font-display)", Sans: "var(--font-ui)" };

/* a single positioned, editable text layer */
function Layer({ layer, selected, onSelect }) {
  const st = {
    left: layer.x + '%', top: layer.y + '%', width: (layer.w || 80) + '%',
    fontFamily: FONT_MAP[layer.font] || FONT_MAP.Sans,
    fontSize: layer.size + 'cqw',
    fontWeight: layer.weight, color: layer.color, textAlign: layer.align,
...
function SocialCard({ card, selectedLayer, onSelectLayer, onBgClick }) {
  return (
...
function makeCarousel(brandHandle) {
  const h = brandHandle || '@still.studio';
  const mk = (id, bg, layers) => ({ id, bg, layers });
  return [
    mk('c1','cover',[
...
function makeSinglePost(brandHandle) {
...
Object.assign(window, { BRANDS, TEMPLATES, PROJECTS, CardBg, SocialCard, Layer, makeCarousel, makeSinglePost, FONT_MAP });
